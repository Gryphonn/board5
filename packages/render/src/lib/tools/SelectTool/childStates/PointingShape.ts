...
export class PointingShape extends StateNode {
  ...
  override onEnter(info: TLPointerEventInfo & { target: "shape" }) {
    ...
    const outermostSelectingShape = this.editor.getOutermostSelectableShape(
      info.shape
    );
  ...
  override onPointerUp(info: TLPointerEventInfo) {
    ...
    if (!this.didSelectOnEnter) {
      ...
      if (selectedShapeIds.includes(outermostSelectableShape.id)) {
          ...
          if (selectedShapeIds.includes(selectingShape.id)) {
            ...
            if (selectedShapeIds.length === 1) {
              const geometry = this.editor
                .getShapeUtil(selectingShape)
                .getGeometry(selectingShape);
              const textLabels = getTextLabels(geometry);
              const textLabel =
                textLabels.length === 1 ? textLabels[0] : undefined;
        ...
        const ancestors = this.editor.getShapeAncestors(
        ...
        this.editor.setSelectedShapes([
          ...this.editor
            .getSelectedShapeIds()
            .filter((id) => !ancestors.find((a) => a.id === id)),
