...
export class Brushing extends StateNode {
  ...
  override onEnter(info: TLPointerEventInfo & { target: "canvas" }) {
    ...
    this.cleanupViewportChangeReactor = react(
      "viewport change while brushing",
      () => {
  ...
  private hitTestShapes() {
    ...
    const brushBoxIsInsideViewport = editor
    ...
    if (
      current.length !== results.size ||
      current.some((id) => !results.has(id))
    ) {
