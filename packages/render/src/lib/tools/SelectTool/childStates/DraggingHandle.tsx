...
export class DraggingHandle extends StateNode {
  ...
  override onEnter(info: DraggingHandleInfo) {
    ...
    if (info.handle.snapReferenceHandleId) {
      const customHandle = handles.find(
        (h) => h.id === info.handle.snapReferenceHandleId
      );
      if (customHandle) {
    ...
    if (this.editor.isShapeOfType(shape, "arrow")) {
      const initialBinding = getArrowBindings(this.editor, shape)[
        info.handle.id as "start" | "end"
      ];
    ...
    if (startChanges) {
      this.editor.updateShapes([
        { ...startChanges, id: shape.id, type: shape.type },
      ]);
    }
