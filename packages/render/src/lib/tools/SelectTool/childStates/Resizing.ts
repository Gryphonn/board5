...
export class Resizing extends StateNode {
  ...
  private updateShapes() {
    ...
    if (shapeSnapshots.size === 1) {
      ...
      if (this.editor.isShapeOfType(onlySnapshot.shape, "text")) {
        isAspectRatioLocked = !(
          this.info.handle === "left" || this.info.handle === "right"
        );
      }
    }
    ...
    const scaleOriginPage = Vec.RotWith(
      altKey
        ...
        : selectionBounds.getHandlePoint(scaleOriginHandle),
    ...
    const distanceFromScaleOriginNow = Vec.Sub(
      currentPagePoint,
      scaleOriginPage
    ).rot(-selectionRotation);

    const distanceFromScaleOriginAtStart = Vec.Sub(
      originPagePoint,
      scaleOriginPage
    ).rot(-selectionRotation);

    const scale = Vec.DivV(
  ...
  private _createSnapshot() {
    ...
    const canShapesDeform = ![...shapeSnapshots.values()].some(
      (shape) =>
        !areAnglesCompatible(shape.pageRotation, selectionRotation) ||
