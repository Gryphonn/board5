...
export class Translating extends StateNode {
  ...
  protected handleStart() {
    ...
    this.dragAndDropManager.startDraggingShapes(
      ...
      compact(
        this.snapshot.movingShapes.map((s) => this.editor.getShape(s.id))
      ),
  ...
  protected handleEnd() {
    ...
    if (this.isCloning && movingShapes.length > 0) {
      const currentAveragePagePoint = Vec.Average(
        movingShapes.map((s) =>
          this.editor.getShapePageTransform(s.id)!.point()
        )
      );
      const offset = Vec.Sub(
        currentAveragePagePoint,
        this.selectionSnapshot.averagePagePoint
      );
      if (!Vec.IsNaN(offset)) {
...
function getTranslatingSnapshot(editor: Editor) {
  ...
  if (onlySelectedShape) {
    initialSnapPoints = editor.snaps.shapeBounds.getSnapPoints(
  ...
  const allHoveredNotes = shapeSnapshots.filter(
    (s) =>
      editor.isShapeOfType(s.shape, "note") &&
...
export function moveShapesToPoint({
  ...
  if (isSnapping && editor.inputs.getPointerVelocity().len() < 0.5) {
    ...
    if (noteSnapshot && noteAdjacentPositions) {
      ...
      const pageCenter = noteSnapshot.pagePoint
        ...
        .add(
          NOTE_CENTER_OFFSET.clone().mul(scale).rot(noteSnapshot.pageRotation)
        );
  ...
  editor.updateShapes(
    compact(
      shapeSnapshots.map(
        ({ shape, pagePoint, parentTransform }): TLShapePartial | null => {
