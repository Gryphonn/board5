...
export class PointingArrowLabel extends StateNode {
  ...
  override onEnter(
    ...
    if (!labelGeometry) {
      throw Error(
        `Expected to find an arrow label geometry for shape: ${shape.id}`
      );
    }
  ...
  override onPointerMove() {
    ...
    const defaultLabelPosition = getArrowLabelDefaultPosition(
      this.editor,
      shape
    );
    ...
    const nextLabelPoint = transform.applyToPoint(
      geometry.interpolateAlongEdge(nextLabelPosition)
    );
